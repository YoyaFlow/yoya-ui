...
describe('className naming contract', () => {
  it('keeps every preset class literal on the yoya-v / yoya-<feature> families', () => {
    // 旧的共享基类 `yoya-component` 已随票 15 删除，这里**不再豁免**它：
    // 谁把类名身份写回来，当场红。
    const violations = classTokens.filter(
      (token) => !COMPONENT_CLASS.test(token) && !UTILITY_CLASS.test(token)
    );
    expect(
      violations,
