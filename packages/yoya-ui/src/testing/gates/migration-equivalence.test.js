 ...
 * 2. `UPDATE_MIGRATION_GOLDEN=1 npx vitest run src/testing/gates/migration-equivalence.test.js` 生成金标，
 *    粘贴进 `GOLDEN`（金标文件在 `packages/yoya-ui/src/testing/baselines/migration-golden.txt`）；
 * 3. 迁移；4. `npx vitest run src/testing/gates/migration-equivalence.test.js` 必须仍然全绿。
 */
...
const GOLDEN_FILE = join(
  ...
  '../baselines/migration-golden.txt'
);
