import { keySet } from '@yoyaflow/yoya-core/internal/core/key-set.js';
import { computed, isSignal, ref } from '@yoyaflow/yoya-core/internal/core/signals/handle.js';
import { vNode } from '@yoyaflow/yoya-core/internal/core/v-node.js';
import { caption, div, table, tbody, td, tfoot, th, thead, tr } from '@yoyaflow/yoya-core/html';
import {
...
export function VTable({ caption, ...rest } = {}) {
  ...
  return vNode((api) => {
    /** 字符串 / 数字 = 表格标题（`caption` 命令与部件一起定义在结构里）。 */
    api.setupString = (value) => api.caption(value);

    return div({ ...rest, vn: 'VTable' }, (shell) =>
      shell.child(
        vTableScroll((scroll) =>
          scroll.child(
