...
import { allocateNumber } from '@yoyaflow/yoya-core/internal/core/id.js';
...
export function vPagination(first = null, second = null, third = null) {
  ...
  if (hasComponentIdentity(args.first, 'VPagination')) {
    // 复用同类实例：options / callback 补上就返回（第二参只落元素级配置，与旧口径一致）
    if (args.options) {
      applyElementOptions(args.first, args.options);
    }
    applySetupValue(args.first, args.callback);
    return args.first;
  }

  // 形态 B → vNode：状态与命令搬进 setup 的闭包，视图（root）由 setup 返回
  let root = null;
  const node = vNode((api) => {
    ...
    // 命令面：写法与旧的形态 B api 一致，只是改成挂到 vNode 的 api 上
    Object.assign(api, {
      change(handler) {
