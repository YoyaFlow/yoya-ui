 ...
 *
 * **这一层不认识任何打包器**：`yoyaPluginFactory(options)` 返回的是标准 Rollup 插件形状
 * （`name` / `enforce` / `transform` / `resolveId` / `load`），所以
 * - Rollup 与 Vite 直接用（`@yoyaflow/yoya-compiler/rollup`，**不依赖 unplugin**）；
 * - Webpack / esbuild / Rspack / Rolldown / Farm 由 `unplugin` 包装一次（`./plugin.js`），
 *   见 `unplugin-bridge.js` 的说明。
 *
 * 使用者在已有的构建配置里加一行，不需要项目脚本、也不需要认识编译产物：
 *
...
/**
 * 插件工厂：返回标准 Rollup 插件对象。产物通过 `onArtifact(name, source)` 可选回调暴露
 * （测试 / 调试）——不挂在插件对象上，因为 esbuild 会校验收到的插件对象、多一个属性就报错
 * （0.6.7 实机踩到）；unplugin 包装层（`./plugin.js`）也照此把回调留在 options 里。
 */
export function yoyaPluginFactory(options = {}) {
  const {
    core,
    coreSpecifier,
    onArtifact,
