...
const runtimePath = join(process.cwd(), 'packages/yoya-core/src/core/compiler-runtime.js');
...
describe('compiled path equivalence', () => {
  ...
  it('builds the same DOM with the node mode', async () => {
    ...
    expect(nodeCompiled.module).toContain('from "@yoyaflow/yoya-core";');
...
describe('live attribute placeholders', () => {
  it('removes the fragment placeholder when a live attribute resolves to nothing', async () => {
    const source =
      "import { computed, div, vText } from '@yoyaflow/yoya-core';\n" +
