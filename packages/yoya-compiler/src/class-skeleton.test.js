...
const surfaceFile = posix.join('packages/yoya-compiler/src/fixtures/card-skeleton.js');
/** 「读不懂的构造体」夹具（与 `card-skeleton.js` 配对）——中性形状，不随包发布。 */
const COMPILER_FIXTURE_FILE = posix.join(
  'packages/yoya-compiler/src/fixtures/unreadable-constructor.js'
);
const surfaceSource = readFileSync(surfaceFile, 'utf8');
// 虚拟夹具路径（文件不落地，但必须与 card-skeleton.js 同目录，编译器按它解析 `./card-skeleton.js`）
const callerFile = posix.join('packages/yoya-compiler/src/fixtures/item-with-card.js');
const callerSource =
