...
const componentFile = posix.join('packages/yoya-compiler/src/fixtures/status-dot.js');
const callerFile = posix.join('packages/yoya-compiler/src/fixtures/item-with-component.js');
...
const themeFile = posix.join('packages/yoya-compiler/src/fixtures/theme-tag.js');
...
describe('static values in linked components', () => {
  it('folds library constants and theme helpers before linking the fragment', () => {
    ...
    expect(compiled.plan.html).toContain('class="yoya-theme-tag"');
