...
/**
 * Parent-shortcut DSL surface merged onto HtmlElementNode: every HTML factory plus the SVG
 * shortcuts registered through registerChildFactories.
 *
 * 组件域的父快捷方法**不写在这里**（票 06 硬点 1）：core 的声明不认识组件，由组件包用
 * `declare module '@yoyaflow/yoya-core/html'` 把各自的 `*ParentShortcuts` 增强进来——与运行期
 * "组件包被 import 时 registerChildFactories"同向。
 */
export interface HtmlElementNode extends ElementNode, HtmlElementShortcuts, SvgParentShortcuts {}
