/**
 * 编译产物的**运行期**钩子（子入口 `@yoyaflow/yoya-core/compiler-runtime`）。
 *
 * 它跑在运行期（不是构建期）：只有**编译过**的项目才会加载它——没有编译路径的项目一个字节都不带。
 * 依赖面只有 core 自己的原语（节点 / 信号 / 键集 / setup 键分类），所以它属于慢线包。
 *
 * 编译器的**名字契约**在这里：导出的名字集合 = 发射器可能写出的钩子全集
 * （用例见 `packages/yoya-compiler/src/runtime-exports.test.js`）。
 */
/**
...
import { computed, isSignal } from './signals/handle.js';
 ...
 * （`packages/yoya-core/src/core/setup-keys.js`）落到既有元素上，键序照对象自身的键序——与核心 `_setupObject` 同表同序。
 *
