...
| Reading `document` / `window` inside a function-value binding (`attr('x', () => window.innerWidth)`)                                          | Keep binding functions pure: depend only on source data (`scope()`, host state, per-request data)                                                                            | Bindings are evaluated once during the server build, so DOM access throws               |
| Reading module-level mutable data from a binding (`() => store.count`, `store` at module scope)                                               | Create the data per request and inject it with `scope(() => store)` or host state                                                                                            | A zero-argument closure declares no source, so module-level data leaks between requests |
