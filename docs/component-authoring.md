...
- **Error boundary**: `node.whenFailed(handler)` declares a subtree boundary — returning a node replaces the subtree with a fallback, returning null only reports and keeps the current state; component objects may define a `whenFailed(error, info)` member next to `render()`, which ComponentNode mounts automatically. Captures are never silent: console.error always fires and a devtools 'error' event is emitted when enabled; nested boundaries stop at the innermost one that takes over, a null return chains outward, and a throwing handler is forwarded to the outer boundary. Without a boundary, errors propagate unchanged (fail fast).
