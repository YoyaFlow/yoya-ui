...
以下为将要实现的特性清单，按领域分组，状态为**待实现**。已发布能力见各文档（SSR 高层入口 `renderPage` / `hydrateOrMount` 已可用，作为本清单中 SSR 基建项的基础）。
...
| 流式渲染 | 页面分块输出 HTML（`<head>` → 骨架 → 内容），配合大页面 maxNodes 回退，缩短首字节时间 | 待实现 |
