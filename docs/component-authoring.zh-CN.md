...
### 6.3 一个键驱动多行：`createKeyedSet()`

6.2 讲的是「共享句柄 + 每行派生」的退化。框架为此提供了**状态层**的替代原语：把「谁被选中 /
激活 / 悬停 / 命中过滤」交给一个按键集合维护，写入只落在**旧键与新键**两个桶上。

```js
const active = createKeyedSet();
// 每一行：值位置照旧（class / attr / style / text / props 都能直接用）
line.toggleClass('danger', active.has(row.id));
// 切换选中：O(1)，只唤醒两个桶
active.set(row.id);
```

实测（`npm run perf:selection`，1000 行）：共享句柄 + 每行派生的一次切换要跑 **1000 次派生求值 /
0.62 ms**；`createKeyedSet()` 是 **0 次派生 / 0.024 ms**（1 万行下分别是 10000 次 / 1.68 ms 与
0 次 / 0.019 ms）。内存上每行从 725 B 降到 530 B（浏览器口径，1000 实例）。

| 用法              | 写法                                                         |
| ----------------- | ------------------------------------------------------------ |
| 单选              | `active.set(id)`                                             |
| 多选集合          | `active.add(id)` / `active.remove(id)` / `active.toggle(id)` |
| 全选 / 批量       | `active.replace(ids)`（只写差集，不会退化成 O(n²)）          |
| 清空选择          | `active.clear()`                                             |
| 命令式 / SSR 判断 | `active.isActive(id)`（纯布尔，不建立订阅）                  |

- **生命周期**：集合与它描述的数据集同寿命。数据里删掉一行时 `active.drop(id)`，
  整体换数据时 `active.dispose()`——桶是按键持有的（每个被观察的键一个薄信号），
  这是有界性的来源。
- **`has(id)` 返回只读句柄**：写入必须走集合方法，否则集合的成员表会与实际值失配。
- **什么时候仍然用 `computed`**：一行依赖多个条件（「选中 且 未禁用」）、需要拼接/运算/分支、
  或值本身不是「键相等」而是更复杂的派生——这些本来就该逐行求值。
- 与 6.2 的关系：6.2 的「行自己持 `ref`」是不引入新 API 的手写方案；本原语把它交给框架维护，
  适合选中态会被多处修改（键盘导航、全选、数据刷新）的列表。
