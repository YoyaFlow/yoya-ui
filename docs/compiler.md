...
| `const view = <factory>(…); return view;` (function body / `vNode` setup / `render()`) | **Named root / assignment flow**: where the block lives, what it is called and how often it is assigned do not take part in the decision (shadowing excepted); reading it elsewhere only switches the channel                      |
| `cell.child(<element factory>(options, setup))`                                        | Same as the prefix form `cell.span(…)`: one argument analysis (options / callback / text / trailing dynamic argument)                                                                                                              |
| `child(<value>)` inside `if` / `for…of` / `forEach`                                    | A **hole**: the call runs verbatim on the product node (core `child()` dispatch), then the new children are placed at the fragment boundary; node handle names in the control statement are rewritten to the product handle `node` |
| `child(vText(x).mountable(cond))`                                                      | **Conditional mount**: placement respects the mount state — an unmounted subtree is not placed into the DOM, and one that is there gets removed (same rule as `keyedRows.place()`)                                                 |
| A third-party **element factory** (custom tag / custom host)                           | Declare it with `markElementFactory(fn, tag)` (symbol key, non-enumerable); components need no mark — their identity is `vn`                                                                                                       |
...
| Construct                                                               | Reason                                                                                                                                                                                                                                                        |
| ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `while` / `for(;;)` / `switch` statements                               | Only `if` / `for…of` / `forEach` are recognised (see the table above); other control statements have no constant iteration count / branch structure                                                                                                           |
...
| A logic frame / hole / control statement **assigning to an outer name** | The product receives a **value** (`scope` destructured into `const`): the write either throws (`Assignment to constant variable`) or never reaches the outer binding (commands keep reading the old value) — `VSymbolButton`'s `rootNode = root` is this case |
| The wrapper around the view is **not core's `vNode`**                   | Core `vNode` is recognised by **name + source** (a renamed alias counts); a local `wrap(…)` / `once(…)` / `memo(…)` always falls back ("recognise wrappers by shape" is pending, see ticket 21 §5)                                                            |
| A **component-body local** in a value position of the `element` channel | That channel carries values, not closures; such a unit needs the node product, so it falls back                                                                                                                                                               |
| A **component-body local** inside a registry entry                      | An entry module can only `import` the original module and has no closure → such a component is not registered (the call site keeps the generic path instead of referencing `undefined`)                                                                       |
...
### 3.1 Profit gate: **depth is decided by profit, not by coverage**

"Can it compile" is a **shape** question; "is it worth compiling" is a **profit** question — they
are answered separately:

- **Without the compiler the behaviour must match**: the compiled path only consumes runtime
  semantics and falls back whenever it cannot read a shape (contract in §6);
- **Performance thresholds come from real machines only**: the same shapes are **slower** when
  swapped in jsdom (1.2–1.6×) and 0.3–0.6× in a real browser — so jsdom keeps the _ordering_ and its
  absolute numbers are void;
- The set of profitable shapes comes from `npm run report:shapes` (headless Edge / Chromium; the
  result lands in `benchmark/shapes.json`; the run compares both columns byte-for-byte first and
  exits non-zero on any mismatch).

Measured (2026-09-23, i7-14700HX / HeadlessChrome 153): construction ops **0.51–0.57×**,
reconciliation ops **0.90–0.94×** (no regression); the larger the row the bigger the win — 4 columns
per row **0.44×** → 8 **0.31×** → 16 **0.24×** → 32 **0.17×**. In other words **the depth threshold
is very low on a real machine (4 columns is already 0.44×) and there is no "too small to compile"
