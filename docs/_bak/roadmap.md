...
以下为特性清单，按领域分组，状态为**已发布**、**待实现**或**不纳入（设计取舍）**。已发布能力见各文档（SSR 高层入口 `renderPage` / `hydrateOrMount`、DevTools 运行时入口与参考面板均已可用）。
...
| 特性                    | 说明                                                                    | 状态   |
| ----------------------- | ----------------------------------------------------------------------- | ------ |
| devtools 运行时入口     | 独立子路径 + 类型声明，默认关闭、零运行时开销，主入口不导出             | 已发布 |
| 视图树快照与节点身份    | 纯数据快照 + 稳定 id + 按 id 定位真实 DOM                               | 已发布 |
| 细粒度更新事件          | attr / style / child / text / destroy 与 state 事件，单调 seq 与节点 id | 已发布 |
| 状态与作用域可观测      | vStateNode 状态事件；access / Context / i18n 详情                       | 已发布 |
| 参考调试面板            | 示例站视图树、详情、事件时间线与 DOM 高亮（只消费公开入口）             | 已发布 |
| 浏览器扩展 / 面板产品化 | React DevTools 级 UI、时间旅行、跨会话持久化                            | 待实现 |

## 说明

- 特性按可独立交付的切片推进，每个切片含测试与演示；
- 不破坏既有 API：底层原语（`renderToString` / `hydrate` / `mount`）保持可用；
- 更多基建项（vite 插件、公开组件规范、AI 生态）将陆续补充到本清单。
