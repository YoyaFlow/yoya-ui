# Spec：Super Table 与基础能力补全（vSuperTable / vTreeTable / vConfirm + Context / devtools / 无障碍 / i18n 纵深）
...
中后台开发者最常见的诉求集中在「高级表格、树形表格、确认操作」三处，而这套库目前把它们留给了使用者自己拼装：

- 现有 `vTable` 是纯展示表（columns/rows/自定义渲染/空态），排序、筛选、行选择、分页联动、编辑、拖拽列都还没有；
...
一次性交付三个组件（`vSuperTable`、`vTreeTable`、`vConfirm`）和四块基础能力（`Context`、`devtools`、`无障碍`、`i18n 纵深`）。组件与能力都以「声明式 DSL + 现有生命周期」为外部形态，复用既有 `setup` 回调、`child()` 组合、SSR 确定性、access/i18n 作用域栈等约定，不引入新渲染模型。

- `vSuperTable`：列配置驱动的增强表格，在 `vTable` 之上叠加排序、筛选、行选择、分页/加载联动、行展开、单元格编辑，并复用 `vScroll` 的虚拟滚动能力。
...
1. 作为页面作者，我想对每列声明 `sorter`，以便点击列头排序，且同列再点可反向/取消排序。
2. 作为页面作者，我想对每列声明 `filter`（预设/远程），以便列头过滤并保持其它列与分页状态一致。
3. 作为页面作者，我想开启行选择（单选/多选），以便表格支持批量操作，且跨分页记住已选 key。
4. 作为页面作者，我想表格与 `vPagination` 联动（服务端/客户端分页），以便大表只渲染当前页。
5. 作为页面作者，我想对可编辑列启用单元格编辑，以便行内改值并校验、提交流出。
6. 作为页面作者，我想对行声明 `expandable` 展开详情，以便展示主行之外的次级信息。
7. 作为页面作者，我想声明列锁定/拖拽调序，以便长表固定关键列、按需调整列宽。
8. 作为页面作者，我想复用 `vScroll` 虚拟滚动，以便超大数据量只渲染可见行。
9. 作为页面作者，我想表格行为在 SSR 与客户端一致，以便首屏可服务端输出、节水合成本。
10. 作为组件作者，我想 `vSuperTable` 暴露稳定的 columns/rows/data API 与事件，以便业务封装与第三方复用。

### vTreeTable

11. 作为页面作者，我想传入树形数据渲染带缩进的表格，以便层级信息在列中并列展示。
12. 作为页面作者，我想点击行首图标展开/折叠子树且 DOM 不重建，以便状态与焦点不丢。
13. 作为页面作者，我想节点选择与父/子联动（选中祖先联动后代），以便批量操作语义正确。
14. 作为页面作者，我想懒加载子树，以便组织/字典这类大层级按需加载。
15. 作为页面作者，我想树表在 SSR 输出确定且可 hydrate，以便与服务端渲染共存。

### vConfirm

16. 作为页面作者，我想用 `vConfirm({ ... })` 一步弹出确定/取消，以便删除等危险操作一致化。
17. 作为页面作者，我想确认按钮支持 loading 与危险态，以便异步删除不重复提交。
18. 作为页面作者，我想回调收到确认结果并可链式，以便页面流程简洁。
19. 作为页面作者，我想 `vConfirm` 在 SSR 下不渲染（纯客户端命令），以便服务端输出不含浮层。

### Context

20. 作为页面作者，我想用 `withContext(provider, build)` 注入「当前用户/门店/品牌主题实例」等跨层数据，以便任意深度的组件就近读取。
21. 作为页面作者，我想子节点就近覆盖祖先 Context，以便局部差异化而不用层层传参。
22. 作为组件作者，我想在构建期捕获 Context 并随渲染生效，以便组件与 SSR 保持同一套作用域。
23. 作为 SSR 使用方，我想入口 `options.context` 按请求注入且不跨请求共享，以便与 i18n/access 同构。

### devtools

24. 作为调试者，我想在开发期看到当前视图树快照，以便快速定位节点/组件结构。
25. 作为调试者，我想查看节点更新事件（属性/子项/销毁），以便追踪一次交互改了哪里。
26. 作为库作者，我想 devtools 默认关闭、按需 enable，以便生产零开销。
27. 作为使用者，我想 devtools 钩子不影响 SSR 与既有版本行为，以便可安全接入浏览器扩展。

### 无障碍

28. 作为视障用户，我想对话框/浮层获得 focus-trap 与正确 `role/aria-modal`，以便键盘循环在弹层内、Esc 关闭。
29. 作为键盘用户，我想菜单/表格/树/标签页支持方向键导航，以便不依赖鼠标。
30. 作为屏幕阅读器用户，我想动态区域（toast/loading）用 `aria-live` 正确播报，以便状态变化可感知。
31. 作为组件作者，我想复用 focus-trap 与键盘原语，以便新组件默认具备基本可访问性。
32. 作为维护者，我想有契约测试守卫 a11y 结构，以便回归不倒退。

### i18n 纵深

33. 作为多语言使用者，我想引用带复数的好消息，以便英文 `1 day / 2 days`、中文无复数时回退正确。
34. 作为使用者，我想数字/日期按 locale 用 `Intl` 格式化，以便 `1000`、`2026-09-03` 随语言呈现。
35. 作为使用者，我想按需注册 lazy locale，以便首屏不加载全部词典。
36. 作为 SSR 使用方，我想语言探测（cookie/header/url）已有基础，以便配合新能力稳定输出同构 HTML。
...
- **vSuperTable（data-display 模块）**：作为 `VTable` 的增强超集，新增以 `TableColumn` 扩展定义的列能力（`sorter`、`sortDirections`、`filter`/`filterOption`、`render`、`editable`、`fixed`、`width`、`ellipsis`），并新增行能力（`rowKey`、`rowSelection`、`expandRowRender`、`onCell`）。数据流为「列配置 + 数据 → 内部有序/过滤后的 rows」，所有排序过滤是可中断的纯函数，确保 SSR 与客户端行为一致。
- **vSuperTable 交互**：排序/筛选由组件内部 reducer 式状态驱动（当前列、方向、过滤条件），触发 `change({ sort, filters, pagination, selectedKeys })` 通知；分页默认客户端，可切换服务端由外部数据源驱动。行选择跨分页用 `rowKey` + 选中 key 集合，不用 index。
...
- **vSuperTable**：客户端排序/取消、筛选组合、跨分页选中保留、单元格编辑提交与校验、虚拟滚动下可见行集合、SSR 输出确定且可 hydrate；先例见 vTable/vScroll 相关测试。
...
- `vSuperTable` 首轮不含：拖拽列宽持久化、列合并（跨行跨列）、服务端筛选的远程协议自动生成。
...
- 各分片建议按 `vConfirm > 无障碍原语 > Context > vSuperTable > vTreeTable > devtools > i18n 纵深` 的顺序推进（先补地基，再叠组件），但顺序可按团队优先级调整。
