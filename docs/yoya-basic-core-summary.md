...
> 注：本文档是对旧仓库 `yoya-basic` 的历史技术总结，仅供追溯参考。其中"局部增强、后端嵌入优先、不推荐 React/Vue 生态、完整 SPA 是暂缓能力"等表述，已被 yoya-ui 的当前目标取代：**yoya-ui 以浏览器原生能力替代 React/Vue 类框架，完整支持整站 SPA，零构建直接运行**。
...
> 以浏览器原生能力替代 React/Vue 类框架的 UI 库：用声明式 DSL 构建可嵌入、可组合、零构建、完整支持整站 SPA 的 Web UI。
