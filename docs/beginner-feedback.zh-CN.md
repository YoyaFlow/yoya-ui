...
| ------------ | ---- | ------------------------------------------------------- |
| ① 旧直觉化石 | 5    | 3, 7, 8, 40, 44                                         |
| ② 换形态     | 34   | 其余（19 / 30 / 39 / 43 带轻度缺失标注，31 为回填转档） |
| ③ 真缺失     | 5    | 20, 23, 27, 41, 42                                      |
...
- 分类：③
- 现状：没有深层 proxy `reactive`，**已定为不做**（依赖收集只认句柄读；深代理要扩引擎契约 `createSignal / read / write / subscribe`、会污染 keyed 的行身份判据，还要维护第二套语义）。对象可以放进 ref（浅层）；推荐拆字段 ref + `computed` 派生；需要 store 形态可 `installSignals` 换外部引擎（那是换值单元实现，不是换语义）。
- 文档动作：FAQ 把「为什么没有 reactive」写成明确口径，并给字段级更新的两条替代路径（字段本身做成句柄 / 换掉整个引用）。
- 优先级：P1 ☐
...
- 分类：②
- 现状：列表协调用 `keyed()`（按 key 对账，只动变化的行，重排不重建节点）；区域重建成本 = 该子树大小，不随页面总节点数增长；5000 行不该全量渲染——再叠加 `vScroll` 虚拟滚动（data-display 已导出）或 `vTable` 的 `rows()/data()` 数据驱动。
- 文档动作：性能 FAQ 引用 keyed 与 vScroll 用例。
- 优先级：P1 ☐
...
- 分类：②（2026-09-15 回填，原 ③）
- 现状：子树错误边界已内建：`node.whenFailed(handler)` 圈定本节点子树，handler 收 `(error, info)`（`info.phase` = build / render / event / update）——返回节点则降级替换子树，返回 null 仅上报并保持现状；组件对象可写与 `render()` 同层的 `whenFailed` 成员，`ComponentNode` 自动挂载。嵌套边界内层接管即止步、返回空继续向外、handler 自身抛错交给外层；捕获永不静默（`console.error` 必发，devtools 开启时追加 error 事件）。无边界时错误原样传播（fail fast），区域更新失败仍先回滚保旧内容。**页面级兜底**（window.onerror 降级页、SSR 整页失败页）仍是使用方职责。
...
7. 「为什么没有 reactive」明确口径（20，已定不做）+「子树边界怎么用」指路（31，边界本身已落地，只缺 FAQ 措辞）。
...
- watch 便捷封装（19）、异步缓存层（30）、页面级错误边界（31 的残余部分）、日期选择器（38）
- 深层 reactive：**明确不做**（决策已定），文档写明替代写法（20）
