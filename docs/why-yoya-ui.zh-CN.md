...
不需要 Wrapper 或 Adapter；表单、表格、导航、反馈、看板等高频能力则开箱即用。npm、Vite/webpack、
TypeScript、CI/CD 与 SSR 等现代工程能力全部一等支持——去掉的只是框架运行时，不是工程基础设施。
...
| SSR 确定性 | `src/*.ssr.test.js`、[ssr.zh-CN.md](ssr.zh-CN.md)            |
...
- [interop.zh-CN.md](interop.zh-CN.md)：支撑"不需要 Wrapper"这一说法的第三方接入范式。
