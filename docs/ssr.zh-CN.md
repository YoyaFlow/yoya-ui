...
### 8.1 序列化输出是规范序：属性与样式都按名字排序

`toHTML()`（以及 SSR 输出）按**属性名排序**写出，与 builder 的写入顺序无关；客户端首帧也用同一个顺序把属性
快照落盘，因此真实元素的属性顺序与服务端 HTML 逐字节一致（之后对同名属性的写入只换值、保持位置）。

这是刻意的：序列化结果可以**只由结构决定**，模板 / 编译器不必复刻你的调用顺序就能产出同一份片段。
需要留意的后果：原先依赖旧写入顺序的 HTML 快照与 `outerHTML` 字符串比较要跟着改（属性与取值本身没变），
`style` 声明同样按序输出。
