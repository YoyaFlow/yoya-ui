...
| 函数值绑定里读 `document` / `window`（`attr('x', () => window.innerWidth)`）         | 绑定函数保持纯函数：只依赖来源数据（`scope()` / 宿主 state / 每请求数据）                                           | 绑定在构建期会在服务端求值一次，DOM 依赖会直接抛错     |
| 函数值绑定读模块级可变数据（`() => store.count`，`store` 在模块顶层）                | 每请求创建数据并用 `scope(() => store)` / 宿主 state 注入                                                           | 零参闭包不声明来源，模块级数据同样会在并发请求间串数据 |
