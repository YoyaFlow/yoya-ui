...
上表的数据来源是 `--report`（本机刷新跑 `npm run report:compile`）：库内 `src` 449 文件 /
候选 61 / 可编 10（`svg/icons.js` 这类真组件也在内），`src/examples` 135 / 50 / 0。
剩下的候选集中在一个形状上：**「调用链不是从 setup 参数出发」**——把节点存进变量（
`const body = root.div(…)` 之后 `body.attr(…)`）或先赋值再挂（`child(v)`），票据里排在
「节点变量 + 按源码偏移的步进表」那一块。这些数字由 §4.1 的覆盖率基线**逐文件**盯着
（基线里可编、现在回落即失败）。
