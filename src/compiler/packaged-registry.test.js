...
const LOCAL_ENTRIES = {
  '@yoyaflow/yoya-ui/core': 'src/yoya.core.js',
  '@yoyaflow/yoya-ui/ui': 'src/yoya.ui.js',
  '@yoyaflow/yoya-ui/data-display': 'src/yoya.data-display.js',
  '@yoyaflow/yoya-ui/compiler-runtime': 'src/yoya.compiler-runtime.js'
};
...
describe('随包注册表的产物口径（票 11）', () => {
  it('按形状扫出来的可编集合：键按包名归口、条目带 bind/render/hash/plan', async () => {
    ...
    built.compiled.forEach((key) => {
      expect(key.startsWith('@yoyaflow/yoya-ui#'), key).toBe(true);
      expect(key.slice('@yoyaflow/yoya-ui#'.length).includes('/'), key).toBe(false);
    });
    expect(module.coreSpecifier).toBe('@yoyaflow/yoya-ui/core');
  ...
  it('只 import 包内入口（不发 src 路径），纯数据里没有 ops', () => {
    ...
    imports.forEach((specifier) => {
      expect(specifier.startsWith('@yoyaflow/yoya-ui/'), specifier).toBe(true);
    });
    expect(bundle).not.toContain('src/compiler');

    const data = JSON.parse(readFileSync(built.files.data, 'utf8'));
    expect(data.coreSpecifier).toBe('@yoyaflow/yoya-ui/core');
...
describe('端到端：业务源码链接库内组件', () => {
  ...
  const source = [
    "import { div, vText } from '@yoyaflow/yoya-ui/core';",
    "import { ArrowDownOutlined } from '@yoyaflow/yoya-ui/core';",
    '',
  ...
  it('经注册表链接后 DOM 与通用路径一致', async () => {
    ...
    expect(wired.units[0].module).toContain('@yoyaflow/yoya-ui#ArrowDownOutlined');
