 ...
 * - 迁移一刀之后跑 `UPDATE_VIEW_BINDING_BASELINE=1 npx vitest run src/view-binding-baseline.test.js`
 *   下调基线（与 `attribute-migration-baseline.json` 同一套做法）。
 *
 * 注意这是**代理指标**：它盯的是形状，不是语义。真正的验收仍是"状态改完当拍 DOM 就对"
 * （见 docs/component-authoring.md 的对照样例）。示例 / 演示（`src/examples`）按票 15 §3-Q11
 * 在波 6 统一处理，不在这里冻结。
 */
...
/**
 * 只看**组件侧**目录：引擎内部（`src/core` / `src/html` / `src/svg`）的 `syncXxx` 是引擎自己的
...
const BASELINE_FILE = join(dirname(fileURLToPath(import.meta.url)), 'view-binding-baseline.json');
