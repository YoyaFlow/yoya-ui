...
import { QuillEditorExample, vQuillEditor } from './demos/quill-editor.js';
...
const quillDemo = Object.freeze({
  ...
  sourceTitle: 'Quill 胶水组件源码',
  usageImports: ['vNode', { from: './demos/quill-editor.js', names: ['vQuillEditor'] }],
...
export function QuillDocumentationPage() {
  return interopPageFrame({
    ...
    note: 'Quill 只在客户端初始化并经 vClientOnly 挂载，无需 SSR 适配。暗色样式独立在 quill-dark.css（不引用 yoya-ui 配色），由胶水组件跟随页面浅色 / 深色 / 系统切换。',
    demos: [quillDemo]
  });
}
