 ...
 *
 * 迁移一刀后跑 `UPDATE_ATTR_BASELINE=1 npx vitest run src/attribute-migration-baseline.test.js`
 * 下调基线；**数字只能往下走**——出现新文件、或某个文件比基线更高，测试就红。
 * 演示 / 示例（`src/examples`）按票 15 §3-Q11 在波 6 统一处理，不在这里冻结。
 */
...
const BASELINE_FILE = join(
  ...
  'attribute-migration-baseline.json'
);
