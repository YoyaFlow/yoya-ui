...
- **文本位置四种等价写法**：`child(vText(handle))`、`child(handle)`、`ele.text(handle)`、元素工厂 setup 位置的 `div(handle)`（等价 `div((el) => el.child(handle))`）都建立同一个绑定（HTML 元素与 SVG 文本宿主都支持）。区别只在语义：`.text()` 每次调用**追加**一个文本节点，适合构建期设置一次；要反复替换同一处文本，就留一个 `vText()` 句柄用 `textContent(next)`。
