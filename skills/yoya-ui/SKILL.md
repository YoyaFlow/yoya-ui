...
- **条件挂载**：`panel.mounted(cond)`（句柄或零参闭包）——为假脱离文档、为真按子节点槽位回归，ViewNode 与控件状态保留；与 `display` 显隐（看不见但在）、`rebuildable()`（销毁重建）构成三档；`isMounted()` 查询条件状态。
...
`mounted()` 在 setup 期声明（已入树后补挂没有公共入口），绑定登记在父节点；三者的细节与坑位见 references/state.md。
