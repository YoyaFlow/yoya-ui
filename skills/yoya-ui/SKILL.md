...
- **纯占位就传句柄本身，不要包一层**：`vText(count)` / `attr('data-count', count)` / `child(count)`（= `child(vText(count))`）/ `ele.text(count)` / 元素工厂 setup 位置的 `div(count)`。写成 `computed(() => count.value)` 是白包；写成 `count.value` 或 `String(count.value)` 是**死快照**（写完不再更新）。
