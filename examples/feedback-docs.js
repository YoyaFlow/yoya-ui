...
const feedbackDocsDefinitions = Object.freeze({
  message: createFeedbackDocsDefinition({
    ...
    apiRows: [
      ['vMessage({ type, content, text, closable })', '创建一条消息。', "vMessage({ type: 'success', text: '保存成功' })"],
      ...
      ['vMessageContainer({ placement })', '创建消息容器。', "vMessageContainer({ placement: 'top-right' })"],
      ['container.show(content, options)', '显示消息并返回 id。', "container.show('保存成功', { type: 'success' })"],
      ['container.success/error/warning/info(content, options)', '按语义显示消息。', "container.success('保存成功')"],
...
const id = host.success('保存成功', { duration: 3000 });
toast.use(host);`,
    examples: [
      {
    ...
    examplesIntro: '下面三个示例分别展示单条消息、局部消息容器和全局 toast。',
    heading: 'vMessage 消息',
    intro:
...
function createFeedbackDocumentationPage(definition) {
  return {
    render() {
      return section((page) => {
        page.className(`components-route-page components-feedback-docs components-feedback-docs--${definition.key}`);
