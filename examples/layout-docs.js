...
const layoutDocsDefinitions = Object.freeze({
  body: createLayoutDocsDefinition({
    ...
    apiRows: [
      ['vBody({ background, maxWidth, padding, gap })', '创建整页壳。', "vBody({ maxWidth: 1120, gap: 24 })"],
      ['container({ maxWidth, paddingInline })', '在内部再收一层内容宽度。', "container({ maxWidth: 920 })"],
      ['content(...)', '访问内层内容容器并继续布局。', 'page.content((content) => content.vstack(...))']
    ],
  ...
  divider: createLayoutDocsDefinition({
    ...
    apiRows: [
      ...
      ["divider({ orientation: 'vertical' })", '纵向分割线，适合并排动作。', "row.divider({ orientation: 'vertical' })"],
  ...
  flex: createLayoutDocsDefinition({
    ...
    apiRows: [
      ['flex({ align, justify, gap, wrap, direction })', '通用弹性容器。', "flex({ gap: '12px', align: 'center', wrap: true })"],
  ...
  grid: createLayoutDocsDefinition({
    ...
    apiRows: [
      ['grid({ columns, rows, areas, autoFlow, gap })', '固定轨道和明确区域。', "grid({ columns: 3, gap: '12px' })"],
      ['responsiveGrid({ minColumnWidth, breakpoints, gap })', '按视口自动换列。', "responsiveGrid({ minColumnWidth: 180 })"],
      ['breakpoints', '支持数组或对象，按最小宽度选择列数。', "responsiveGrid({ breakpoints: [{ minWidth: 640, columns: 2 }] })"]
    ],
  ...
  spacer: createLayoutDocsDefinition({
    ...
    apiRows: [
      ...
      ["spacer({ size, orientation })", '可固定宽度或高度。', "spacer({ size: 24, orientation: 'vertical' })"],
  ...
  popup: createLayoutDocsDefinition({
    ...
    apiRows: [
      ...
      ['dialog.content(setup)', '继续填充弹窗内部。', 'dialog.content((sheet) => sheet.vstack(...))'],
    ...
    heading: 'vDialog 弹窗',
    intro: 'vDialog 让弹窗成为一个独立的对象组件，外层按钮负责触发，内层内容负责确认、取消和临时表单。',
    key: 'popup',
    routeItem: 'layout:5',
    title: '弹窗',
    usageItems: [
  ...
  templates: createLayoutDocsDefinition({
    ...
    apiRows: [
      ['后台仪表盘', '适合运营总览、监控看板和任务中心。', 'vBody + header + metrics grid + workspace split'],
...
function createLayoutDocumentationPage(definition) {
  return {
    render() {
      return section((page) => {
        page.className(`components-route-page components-layout-docs components-layout-docs--${definition.key}`);
          ...
          content.vstack((stackNode) => {
            ...
            stackNode.hstack((row) => {
              ...
              row.div((title) => {
                title.style('gap', '6px');
                title.h2('服务工作台');
                title.p('vBody 统一页面背景、内容宽度与留白，内部再继续用 container、grid 和 center 组织内容。');
              });
              row.spacer();
