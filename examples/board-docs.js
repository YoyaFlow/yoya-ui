...
const pageConfigs = Object.freeze([
  {
    ...
    extraSource: liveDataSource,
    heading: '数字看板 vDigitalBoard',
    imports: ['computed', 'keySet', 'ref', 'vDigitalBoardItem', 'vstack'],
    intro:
      '用响应式卡片网格展示关键指标：数值、单位、趋势和主题色；示例的指标是 keySet' +
      '（数值 / 上次值挂在各自 api），点「开始模拟」后每 1.2s 变化一次，还能增删指标。',
    key: 'digital-board',
    ...
    usage: [
      '需要在一屏内展示多个关键指标时。',
      '需要趋势方向与主题色自动着色时。',
      '指标值随数据变化时：把 ref / computed 句柄交给 item.value(...)，卡片原地更新。',
      '指标会增删排序时：用 keySet + board.keyed(...)，同 key 同 api、只对账差分行。'
    ],
    apiRows: [
      ...
      [
        'item.value(handle)',
        '指标值传句柄（ref / computed）就是活值，写状态即更新。',
        'item.value(cpuUsage)'
      ],
      [
        'board.keyed(metrics, (item) => …)',
        '指标集合用 keySet：增删排序按 key 对账，数值与上次值挂在 item.api 上。',
        'metrics.remove(key)'
      ]
    ]
  },
  {
    ...
    extraSource: liveDataSource,
    imports: ['computed', 'keySet', 'ref', 'vTrendCard', 'vstack'],
    intro:
      '组合数值、涨跌和迷你走势，适合放在看板顶部；示例的三张卡片都由 keySet 管着，' +
      '「开始模拟」后数值与走势一起变。',
    key: 'trend-card',
    ...
    usage: [
      '需要展示指标与变化方向时。',
      '适合看板顶部摘要卡。',
      '数值 / 走势会变时：把 ref 句柄交给 value / data / up，卡片自己更新。'
    ],
    apiRows: [
  ...
  },
  {
    ...
    extraSource: liveDataSource,
    imports: ['keySet', 'ref', 'vSparkline', 'vstack'],
    intro:
      '无坐标轴的轻量折线图，支持面积填充和主题色；示例每条走势挂一个 ref 数组，' +
      '「模拟一轮」就往后推一个点。',
    key: 'sparkline',
    route: 'board:sparkline',
    title: '迷你走势',
    usage: ['需要在卡片里嵌入小图趋势时。', '数据滚动时：data 传 ref 数组，写数组即重画。'],
    apiRows: [
  ...
  },
  {
    ...
    extraSource: liveDataSource,
    imports: ['computed', 'keySet', 'ref', 'vRingStat', 'vstack'],
    intro:
      '用圆环展示占比，中心默认显示百分比，也可自定义；示例的占比挂在 ref 上，' +
      '容量环的中心文本由 percent 派生。',
    key: 'ring-stat',
    route: 'board:ring-stat',
    title: '环形统计',
    usage: ['需要展示比例类指标时。', '适合成功率、容量占用。', '占比会变时：percent 传句柄。'],
    apiRows: [
  ...
  },
  {
    ...
    heading: '仪表盘 vGauge',
    extraSource: liveDataSource,
    imports: ['keySet', 'ref', 'vGauge', 'vstack'],
    intro:
      '用半圆刻度与指针展示区间指标，支持最大值与单位；示例三个表盘都由 ref 驱动，' +
      '「开始模拟」后指针跟着数据走。',
    key: 'gauge',
    route: 'board:gauge',
    title: '仪表盘',
    usage: ['需要展示区间型指标时。', '适合负载、使用率等。', '区间值会变时：value 传句柄。'],
    apiRows: [
