...
- [ ] `div((ele) => ele.attr('data-x', () => 'v'))` 构建期不报错，首屏 DOM 与 SSR HTML 都有 `data-x="v"`。
- [ ] 上述节点 `flush()` 后重新求值；值未变化时不写 DOM。
- [ ] `(s) => …` 在无来源节点构建期报错，文案指向「参数化值需要 scope 或宿主来源」；零参闭包不报错。
- [ ] `scope(getter)` 声明一次后，多层多分支子树的参数化绑定全部读到该来源，父级一次 `flush()` 刷完整棵树。
- [ ] 组件内不声明任何来源时行为与今天一致（宿主继承），5 个已迁移组件零改动通过回归。
- [ ] `dataSource()` 已删除（或抛错提示），全仓文案、类型、示例同步；两份 skill 字节一致且校验通过。
- [ ] SSR 三条纪律写入 `docs/ssr*` 与 skill；新增两条 SSR 测试通过。
- [ ] `lint` / `format:check` / `typecheck` / `test`（含 13 个区域与状态测试文件）/ `build` 全绿。
