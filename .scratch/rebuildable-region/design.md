...
## 16. 后续方向：统一绑定归属与数据来源（工作票 13）

v1 落地后重新审视「绑定读什么」，发现 scope 对象同时承担了两件事，导致裸节点写任何函数值都报错、独立子树被迫用 `rebuildable()` 换取作用域：

| 职责 | 谁需要 | 是否需要声明 |
| --- | --- | --- |
| **绑定归属**（谁持有、首屏谁求值、flush 谁遍历、destroy 谁释放） | 所有绑定 | 不需要——节点本来就有 `_bindings` |
| **数据来源**（`getState()` 给 `(s) => …` 当 `s`） | 只有参数化绑定 | 需要（`scope(getter)` 或宿主继承） |

结论：**归属隐式化、来源显式化**。`dataSource()` 由 `scope(getter)` 取代并删除（生产代码零使用），`rebuildable()` 只保留「结构可重建」+ 宿主作用域捕获；零参闭包在任意节点可用（首屏求值一次，之后手动 `flush()`），参数化绑定仍要求来源。

完整决策、契约判定表、改动清单与验收项见 `issues/13-unify-binding-source.md`。
